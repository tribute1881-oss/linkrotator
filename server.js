...
app.get('/api/stats', (req, res) => {
  try {
    ...
    const totals = query('SELECT COUNT(*) as total_clicks, COUNT(DISTINCT ip_hash) as uniq_clicks FROM clicks WHERE clicked_at >= ?', [since]);
    const linksCount = query('SELECT COUNT(*) as n FROM links');
    const activeCount = query('SELECT COUNT(*) as n FROM links WHERE enabled = 1');
    const chart = query('SELECT date(clicked_at) as day, COUNT(*) as total, COUNT(DISTINCT ip_hash) as uniq FROM clicks WHERE clicked_at >= ? GROUP BY date(clicked_at) ORDER BY day ASC', [since]);
    ...
    const t = totals[0] || {};
    res.json({ totals: { clicks: t.total_clicks || 0, unique: t.uniq_clicks || 0, links: (linksCount[0] || {}).n || 0, active: (activeCount[0] || {}).n || 0 }, chart: chart || [], topLinks: topLinks || [] });
  } catch(e) {
